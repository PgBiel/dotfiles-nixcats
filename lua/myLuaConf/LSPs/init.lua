...
-- PG: Show hover after cursor is on something for a while
-- See https://old.reddit.com/r/neovim/comments/v8biat/show_vimlspbufhover_on_cursorhold/
            ...
            if not require("cmp").visible() then
                local hover_fixed = function()
                    for _, win in pairs(vim.api.nvim_list_wins()) do
                      if vim.api.nvim_win_call(win, function() return vim.g.filetype == "hovertype" end) then
                        -- don't open again
                        print("okay not trying again")
                        return
                      end
                    end

                    -- Ensure we don't start moving inside the hover window
                    vim.lsp.buf.hover({ focusable = false, title = 'Hover' })
                end
                hover_fixed()
            end
        end
...
-- Completely prevent focus on hover
...
local function custom_hover(_, result, ctx, config)
  ...
  -- PG: custom config here
  local bufnr, winnr = util.open_floating_preview(contents, format, config)
  vim.api.nvim_win_set_var(winnr, 'filetype', 'hovertype')
  return bufnr, winnr
end
